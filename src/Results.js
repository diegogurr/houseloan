...
function Results(props) {
    ...
    const handleAddButtonClick = (minPrice, maxPrice) => {
        ...
        while (price <= maxPrice) {
            ...
            const newItem = createData('For ' + price, calculatePOS(price, 3.04), 0.1 * price, calculatePOS(price, 2.70), calculatePOS(price, 2.58), calculatePOS(price, 2.81))
        ...
        <Box sx={{}}>
            ...
            <TableContainer component={Paper}>
                <Table sx={{minWidth: 650}} aria-label="simple table">
                    <TableHead>
                        <TableRow sx={{backgroundColor: '#e9c46a'}}>
                        ...
                        {rows.map((row) => (
                            <TableRow key={row.name} sx={{'&:last-child td, &:last-child th': {border: 0}}}>
                                ...
                                <TableCell sx={{color: currentIncomesSum > row.paymentPerMonthAPS ? 'green' : 'red'}}
                                           align="right">{row.paymentPerMonthAPS}</TableCell>
